...
folder (yes = write path/index.html), publish (no = keep the page in the build but write nothing public)."""
import os, re, io, sys
...
for name in sorted(os.listdir(P('build','pages'))):
    ...
    if v.get('publish') == 'no':
        skipped.append(v['path']); continue
    ...
    html = html.replace('href="%s" class="mm"' % top, 'href="%s" class="mm current"' % top, 1)
    out = v['path'].strip('/')
...
if skipped: print('not published (publish: no):', ', '.join(skipped))
